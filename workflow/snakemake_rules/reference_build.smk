...
rule download:
    message:
        "Downloading reference sequences and metadata"
    output:
    ...
    params:
        ...
        deflate_seq=lambda w: _infer_decompression(
            config["builds"][w.build_name]["reference_metadata"]
        ),
        deflate_meta=lambda w: _infer_decompression(
            config["builds"][w.build_name]["reference_sequences"]
        ),
    shell:
        """
...
rule combine_input_metadata:
    ...
    run:
        ...
        user_metadata_files = (
            [input.user_metadata]
            if type(input.user_metadata) == str
            else input.user_metadata
        )
        user_metadata = []
        ...
        combined_meta = (
            pd.concat([ref_meta] + user_metadata, axis=0)
            .fillna("?")
            .drop_duplicates(subset="strain", keep="first")
        )
...
rule combine_sequences:
    ...
    input:
        lambda w: [f"reference-datasets/{w.build_name}/sequences.fasta"]
        + (
            [config["builds"][w.build_name]["user_sequences"]]
            if type(config["builds"][w.build_name]["user_sequences"]) == str
            else config["builds"][w.build_name]["user_sequences"]
        ),
    output:
